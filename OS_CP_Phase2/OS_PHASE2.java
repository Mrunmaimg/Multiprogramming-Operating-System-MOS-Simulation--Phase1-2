...
public class OS_PHASE2 {
    ...
    OS_PHASE2(String inputfile, String outputfile) throws Exception {
    ...
    private void printMemory() {
        for (int i = 0; i < M.length; i++) {
            // Print memory index with two digits (e.g., 00, 01, 02)
            System.out.printf("%02d |", i);
            // Print memory contents in the format: "char | char | char | char |"
            for (int j = 0; j < M[i].length; j++) {
                System.out.printf(" %c |", M[i][j] == ' ' ? ' ' : M[i][j]);
            }
            System.out.println();

            // Add a blank line after every 10 rows
            if ((i + 1) % 10 == 0) {
                System.out.println();
            }
        }
    ...
    public static void main(String[] args) throws Exception {
        String InputFile = "inputphase2.txt";
        String OutputFile = "output.txt";

        OS_PHASE2 p1 = new OS_PHASE2(InputFile, OutputFile);
